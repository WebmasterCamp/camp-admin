...
const authStateConnect = connect(state => ({
...
export const redirectIfLoggedIn = path =>
  compose(
    authStateConnect,
    branch(props => !props.isCheckedUser, renderNothing),
    branch(props => props.isLoggedIn, renderComponent(RedirectToPath(path))),
  )

export const redirectIfNotLoggedIn = compose(
  authStateConnect,
  branch(props => !props.isCheckedUser, renderNothing),
  branch(props => !props.isLoggedIn, renderComponent(RedirectToLogin)),
)
