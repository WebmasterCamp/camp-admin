      ...
      <Row gutter={16}>
        {['programming', 'design', 'marketing', 'content'].map(role => (
          <Col span={6}>
            <MajorCard>
              <MajorTitle>
                {majorAsText(role, true)} ({toPassCount[role]})
              </MajorTitle>
              <MajorCount>
                {getSummation(
                  interviewEstimation[role].slice(toPassCount[role]),
                )}
              </MajorCount>
              {interviewEstimation[role]
                ...
                .map((count, index) => (
                  <p>
                    {getSummation(interviewEstimation[role].slice(index))}{' '}
                    candidates for {index} pass
                  </p>
                ))}
            </MajorCard>
          </Col>
