    ...
    return [
      ...col,
      {
        ...
        render: (text, item) => (
          <div>
            <GreenButton
              onClick={() => props.approveSlip(item._id)}
              icon="check-circle"
            ...
            <RedButton
              onClick={() => props.notApproveSlip(item._id)}
              icon="close-circle"
            >
              Not Approve
            </RedButton>
...
const enhance = compose(
  ...
  withProps(props => ({
    approveSlip: id =>
      props.updateSlipState(id, true).then(() => props.getSlips()),
    notApproveSlip: id =>
      props.updateSlipState(id, false).then(() => props.getSlips()),
  })),
  lifecycle({
    componentDidMount() {
      this.props.getSlips()
    ...
    <Collapse defaultActiveKey={['pending', 'confirm', 'not confirm']}>
      ...
      <Collapse.Panel key="confirm" header="Approve">
        <Table
          ...
          columns={columns()}
          dataSource={slips
            .filter(slip => slip.status === 'approve')
            .sort((a, b) => {
