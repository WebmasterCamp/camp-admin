...
const enhance = compose(
  ...
  withProps(props => ({
    onSearch: () =>
      props
        .getInterviewer(props.refID.toUpperCase())
        .then(() => props.setRefID('')),
  })),
  lifecycle({
    componentDidMount() {
      ...
      socket.on('queue', data => {
        ...
        this.props.setQueue(
          `${role[0].toUpperCase()}${_.padStart(data[role], 2, '0')}`,
        )
      })
    },
  }),
...
const Interviewer = props => {
  ...
  return (
    <div>
      <Container>
        <Title>
          คิวหน้าเว็บ (คนสุดท้ายที่อยู่หน้าห้อง):{' '}
          <Highlight>{currentQueue}</Highlight>
        </Title>
        ...
        >
          <Input
            ...
            placeholder="REF ID ex. D01"
          />
          <Button icon="search" onClick={() => props.onSearch()}>
            Search
          </Button>
        </form>
      </Container>
            ...
            interviewer && (
              <div>
                ...
                <QuestionContainer>
                  ...
                  {interviewer.questions.specialQuestions[
                    interviewer.major
                  ].map((answer, idx) => (
                    <QuestionCard
                      key={answer._id}
                      question={
                        questions.specialQuestions[interviewer.major][idx]
                      }
                      answer={answer.answer}
                    />
                  ))}
                  {interviewer.major === 'design' && (
                    <div>
                      ...
                      {interviewer.designPortfolio ? (
                        <a
                          href={getPdfPath(interviewer.designPortfolio)}
                          target="_blank"
                        >
                          ดู Portfolio
                        </a>
