...
const majorAnswerToComponent = (major, props) => (answer, idx) => (
  <QuestionCard
    ...
    monospace={major === 'programming' && idx === 3}
  />
)
...
const Registrant = props => {
  ...
  const {
    ...
    specialQuestions: majorAnswers,
  } = props.registrant.questions
  return (
    <div>
      <Tabs>
        ...
        <TabPane tab="คำถามสาขา" key="3">
          {completedMajor && (
            <MajorTitle>{majorAsText(completedMajor)}</MajorTitle>
          )}
          {completedMajor &&
            majorAnswers[completedMajor].map(
              majorAnswerToComponent(completedMajor, props),
            )}
          {completedMajor === 'design' && (
            <div>
              ...
              {props.registrant.designPortfolio ? (
                <a
                  href={getPdfPath(props.registrant.designPortfolio)}
                  target="_blank"
                >
                  ดู Portfolio
                </a>
          ...
          {!completedMajor &&
            majorAnswers['design'].length !== 0 && (
              <div>
                <MajorTitle>{majorAsText('design')}</MajorTitle>
                {majorAnswers['design'].map(
                  majorAnswerToComponent('design', props),
                )}
              </div>
            )}
          {!completedMajor &&
            majorAnswers['content'].length !== 0 && (
              <div>
                <MajorTitle>{majorAsText('content')}</MajorTitle>
                {majorAnswers['content'].map(
                  majorAnswerToComponent('content', props),
                )}
              </div>
            )}
          {!completedMajor &&
            majorAnswers['programming'].length !== 0 && (
              <div>
                <MajorTitle>{majorAsText('programming')}</MajorTitle>
                {majorAnswers['programming'].map(
                  majorAnswerToComponent('programming', props),
                )}
              </div>
            )}
          {!completedMajor &&
            majorAnswers['marketing'].length !== 0 && (
              <div>
                <MajorTitle>{majorAsText('marketing')}</MajorTitle>
                {majorAnswers['marketing'].map(
                  majorAnswerToComponent('marketing', props),
                )}
              </div>
            )}
        </TabPane>
