  ...
  withProps(ownProps => ({
    lists: ownProps.lists
      .map(item => ({
        key: item._id,
        id: item._id,
        status: _.isEmpty(item.questions.stageThree)
          ? '-'
          : item.questions.stageThree.isPass ? 'Pass' : 'Fail',
        note: _.isEmpty(item.questions.stageThree)
          ? ''
          : item.questions.stageThree.note,
      }))
      .filter(item => {
    ...
    loadGradingItem: id =>
      ownProps
        .getMajorItem(ownProps.user.role, id)
        .then(() => ownProps.setCurrentId(id)),
    getCandidateCount: () => ownProps.getCandidateCount(ownProps.user.role),
    gradedCount: ownProps.lists.filter(
      item => !_.isEmpty(item.questions.stageThree),
    ).length,
...
const columns = props => [
  {
    ...
    render: (text, item, idx) => (
      <a onClick={() => props.loadGradingItem(item.id)}>
...
const MajorList = props => {
  return (
    <div>
      ...
      <CandidateTitle>
        Pass:{' '}
        <CandidateCount exceed={props.candidateCount > 60}>
      ...
      <ProgressContainer>
        <Row>
          <Col span={8}>
            ...
            <Progress
              percent={props.gradedCount / props.allCount * 100}
              format={() => `${props.gradedCount}/${props.allCount}`}
            />
          </Col>
        </Row>
      </ProgressContainer>
            ...
            !props.isLoadingItem && (
              <GradingContainer>
                <MajorGrading
                  ...
                  doneGrading={() =>
                    props
                      .getMajorList(props.user.role)
                      .then(() => props.getCandidateCount())
                      .then(() => props.setCurrentId(''))
                  }
