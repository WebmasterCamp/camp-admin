...
const enhance = compose(
  ...
  withProps(ownProps => ({
    onPass: (pass, note) => {
      ownProps
        .gradeStageOneItem(ownProps.userId, pass, note)
  ...
  return (
    <div>
      ...
      {answers.map((answer, idx) => (
        <AnswerItem answer={answer.answer} question={generalQuestions[idx]} />
      ))}
      ...
      <ActionSpan>
        <GreenButton
          icon="check-circle"
          onClick={() => props.onPass(true, note)}
        >
          Yes
        </GreenButton>
        <RedButton
          icon="close-circle"
          onClick={() => props.onPass(false, note)}
        >
          Nope
        </RedButton>
      </ActionSpan>
