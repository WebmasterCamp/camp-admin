...
const enhance = compose(
  ...
  withProps(ownProps => ({
    lists: ownProps.lists.map(item => ({
      key: item._id,
      id: item._id,
      status: _.isEmpty(item.questions.stageOne)
        ? '-'
        : item.questions.stageOne.isPass ? 'Pass' : 'Fail',
      note: _.isEmpty(item.questions.stageOne)
        ? '-'
        : item.questions.stageOne.note,
    })),
    loadGradingItem: id =>
      ownProps.getStageOneItem(id).then(() => ownProps.setCurrentId(id)),
  })),
  lifecycle({
    componentDidMount() {
          ...
          !props.isLoadingItem && (
            <GradingContainer>
              <StageOneGrading
                ...
                doneGrading={() =>
                  props.getStageOneList().then(() => props.setCurrentId(''))
                }
              />
            </GradingContainer>
          )}
        {props.isLoadingItem && <h1>Loading...</h1>}
      </Col>
    </Row>
  </div>
)
